...
- Build pipeline: `xcodegen generate` → archive Release (manual, both targets) → export with `build/ExportOptions.plist` → `altool --apiKey 8XWLD2B2RQ --apiIssuer 538cb0d4-b8c6-4bc7-8b59-75da5d2b9411` → `asc builds wait` + `asc builds add-groups --group 34067b3b-3cb4-40e7-b4f2-6d02b2f74843`. Bump `CURRENT_PROJECT_VERSION` each build.
