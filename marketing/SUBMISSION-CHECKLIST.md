...
- Build pipeline: `xcodegen generate` → archive Release (manual, both targets) → export `build/ExportOptions.plist` → `altool --apiKey 8XWLD2B2RQ --apiIssuer 538cb0d4-…`. Bump `CURRENT_PROJECT_VERSION` per build.
