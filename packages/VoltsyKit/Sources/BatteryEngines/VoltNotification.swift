...
public enum VoltNotificationEngine {
    ...
    public static func message(level: Double, state: BatteryChargeState, thermal: ThermalLevel,
        ...
        if thermal >= .serious {
            return VoltMessage(kind: .overheating, body: "Phew, I'm getting toasty. Give me a breather? 🥵")
        }
        if state == .unplugged, level >= 0, level <= 0.15 {
            return VoltMessage(kind: .lowBattery, body: "I'm fading… \(pct)% and dropping. Got a charger? 🔌")
        }
        if state == .charging, level >= 0.80 {
            return VoltMessage(kind: .fullChargeNudge, body: "I'm full at \(pct)%. Unplug me to keep my battery young! ✨")
        }
